...
class TextureImage:
    ...
    def __init__(self, picture_data, hotspot=None):
        ...
        dbg("creating TextureImage with size %d x %d" % (
...
def merge_frames(frames, max_width=0, max_height=0):
    ...
    dbg("merging %d frames to %dx%d atlas, %d pics per row, %d rows." % (
        len(frames), width, height, max_per_row, num_rows), 2)
    ...
    for sub_frame in frames:
        ...
        dbg("drawing frame %03d on atlas at %d x %d..." % (
            len(drawn_frames_meta), pos_x, pos_y), 3)
