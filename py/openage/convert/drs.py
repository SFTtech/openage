...
class DRS:
    ...
    see doc/media/drs-files for specification
    """
    ...
    def __init__(self, fname, name):
        ...
        for i in range(table_count):
            ...
            for j in range(file_count):
                file_header = DRS.drs_file_info.unpack_from(
                    file_info_buf, j * DRS.drs_file_info.size)
                ...
                self.files[(file_extension, file_id)] = (
                    file_data_offset, file_size)
    ...
    def __repr__(self):
        return "DRS file (%d tables, %d files)" % (
            self.header[3], len(self.files))
