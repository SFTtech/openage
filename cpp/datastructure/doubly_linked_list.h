...
#include "../util/block_allocator/block_dynamic.h"
...
public:
	...
	node_t *insert_first_item(const T &item) {
		if (this->size() != 0) {
			...
			node_t *new_node   = this->alloc.create(item);
...
protected:
	...
	allocator alloc;   //!< The memory allocator being used by this object
};
