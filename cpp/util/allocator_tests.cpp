...
int block_allocator_size(){
	...
	size_t test1 = (size_t)alloc.get_ptr();
	size_t test2 = (size_t)alloc.get_ptr();
...
int block_allocator_free_test(){
	...
	alloc.get_ptr();
	val_ptr<size_t>* holder = alloc.get_ptr();
	alloc.get_ptr();
	alloc.free(holder);
	if(holder != alloc.get_ptr()){return 0;}
	if((size_t)holder + 2*alloc.alloc_size() != (size_t)alloc.get_ptr()){
		return 1;
	}
	return -1;
}
...
void block_allocator(){
	...
	if((ret = block_allocator_size<char>()) != -1){
		testname = "allocation layout failed for small objects";
	}
	else if((ret = block_allocator_size<char[27]>()) != -1){
		testname = "allocation layout failed for large objects";
	}
...
int stack_allocator_size(){
	...
	char* fst = alloc.get_ptr();
	if(alloc.get_ptr() - fst != 1){return 0;}
	return -1;
}
...
void stack_allocator(){
	...
	if((ret = stack_allocator_size()) != -1){
		...
		testname = "stack block allocation failed";
	}
	else{
		return;
	}
