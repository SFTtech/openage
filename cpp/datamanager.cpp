...
std::shared_ptr<UnitTexture> DataManager::get_unit_texture(index_t unit_id) {
	if (this->unit_textures.count(unit_id) == 0) {
		if (unit_id > 0) {
			log::msg("  -> ignoring unit_id: %d", unit_id);
		}
		return nullptr;
	}
...
Sound *DataManager::get_sound(index_t sound_id) {
	if (this->available_sounds.count(sound_id) == 0) {
		if (sound_id > 0) {
			log::msg("  -> ignoring sound_id: %d", sound_id);
		}
		return nullptr;
	}
...
UnitProducer *DataManager::get_producer(index_t producer_id) {
	if (this->producers.count(producer_id) == 0) {
		if (producer_id > 0) {
			log::msg("  -> ignoring producer_id: %d", producer_id);
		}
		return nullptr;
	}
...
void DataManager::load_building(const gamedata::unit_building &building, producer_list &list) {
	...
	if (this->valid_graphic_id(building.graphic_standing0)) {
		list.emplace_back(util::make_unique<BuldingProducer>(*this, &building));
...
void DataManager::load_living(const gamedata::unit_living &unit, producer_list &list) {
	...
	if (this->valid_graphic_id(unit.graphic_dying0) &&
		...
		list.emplace_back(util::make_unique<LivingProducer>(*this, &unit));
...
void DataManager::load_object(const gamedata::unit_object &object, producer_list &list) {
	...
	if (this->valid_graphic_id(object.graphic_standing0)) {
		list.emplace_back(util::make_unique<ObjectProducer>(*this, &object));
...
void DataManager::load_projectile(const gamedata::unit_projectile &proj, producer_list &list) {
	...
	if (this->valid_graphic_id(proj.graphic_standing0)) {
		list.emplace_back(util::make_unique<ProjectileProducer>(*this, &proj));
...
void DataManager::create_abilities(const std::vector<gamedata::empiresdat> &gamedata) {
	...
	log::dbg("%d headers with %d total commands", headers, total);
}
