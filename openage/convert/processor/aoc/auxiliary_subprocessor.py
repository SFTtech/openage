...
class AoCAuxiliarySubprocessor:
    ...
    @staticmethod
    def get_projectile(line, position=0):
        """
        Creates a Projectile object for a ranged unit/building line.

        :param line: Unit/Building line.
        :type line: ...dataformat.converter_object.ConverterObjectGroup
        :param position: When 0, creates the first projectile. When 1, the second...
        :type position: int
        """
        current_unit_id = line.get_head_unit_id()
        dataset = line.data
        ...
        obj_name = "Projectile%s" % (str(position))
        ...
        # Abilities (created somewhere else)
        abilities_set = []
        proj_ability_ref = "%s.ShootProjectile.Projectile%s.Projectile"\
            % (game_entity_name, str(position))
        abilities_set.append(ExpectedPointer(line, proj_ability_ref))
        ...
        # Variants (created somewhere else)
        variants_set = []
