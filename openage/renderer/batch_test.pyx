...
from libopenage.renderer.batch_test cimport batch_demo as batch_demo_c

def batch_demo(list argv):
    """
    ...
    cmd = argparse.ArgumentParser(
        prog='... batch_demo',
        description='Demo of the new batch renderer')
    ...
    cdef int batch_test_id = args.test_id

    cdef Path_cpp root_cpp = Path_cpp(PyObj(<PyObject*>root.fsobj),
                                  root.parts)

    with nogil:
        batch_demo_c(batch_test_id, root_cpp)
