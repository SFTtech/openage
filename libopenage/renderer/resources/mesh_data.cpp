...
MeshData MeshData::make_alpha(const std::array<float, 24> quad_data) {
	auto const data_size = quad_data.size() * sizeof(decltype(quad_data)::value_type);
	std::vector<uint8_t> verts(data_size);
	std::memcpy(verts.data(), reinterpret_cast<const uint8_t*>(quad_data.data()), data_size);

	VertexInputInfo info { { vertex_input_t::V2F32, vertex_input_t::V2F32,vertex_input_t::V2F32}, vertex_layout_t::AOS, vertex_primitive_t::TRIANGLE_STRIP };
