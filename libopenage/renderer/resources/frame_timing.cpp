...
FrameTiming::FrameTiming(const curve::time_t &total_length,
...
void FrameTiming::set_total_length(const curve::time_t &total_length) {
	this->total_length = total_length;
}

void FrameTiming::insert(const curve::time_t &time) {
...
size_t FrameTiming::get_frame(const curve::time_t &time) const {
	return search_frame(time);
}

size_t FrameTiming::get_frame(const curve::time_t &time, const curve::time_t &start) const {
	curve::time_t offset = time - start;
...
size_t FrameTiming::search_frame(const curve::time_t &time) const {
	size_t left = 0;
