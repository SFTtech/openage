...
void Physics::processInput(PongState &state, PongPlayer &player, std::vector<event> &events, curve::EventQueue *queue, const curve::curve_time_t &now) {
...
void Physics::reset(PongState &state, curve::EventQueue *q, const curve::curve_time_t &gnow) {
	q->clear();
	auto now = gnow + 0.001;
	state.ball.position.set_drop(now, state.resolution * 0.5);
	...
	static int cnt = 0;
	mvprintw(20, 20, "Reset. Speed %f | %f POS %f | %f [%i]", init_speed[0], init_speed[1], pos[0], pos[1], ++cnt);
	predict_reflect_panel(state, q, now);
	predict_reflect_wall(state, q, now);
}


void Physics::ball_reflect_wall(PongState &state, curve::EventQueue *queue, const curve::curve_time_t &now) {
	auto pos = state.ball.position.get(now);
...
void Physics::predict_reflect_wall(PongState &state, curve::EventQueue *queue, const curve::curve_time_t &now) {
	auto pos = state.ball.position.get(now);
	auto speed = state.ball.speed.get(now);
...
void Physics::ball_reflect_panel(PongState &state, curve::EventQueue *queue, const curve::curve_time_t &now) {
...
void Physics::predict_reflect_panel(PongState &state, curve::EventQueue *queue, const curve::curve_time_t &now) {
	auto pos = state.ball.position.get(now);
	auto speed = state.ball.speed.get(now);
