...
GuiBasic::GuiBasic(SDL_Window *window, const std::string &source, qtsdl::GuiSingletonItemsInfo *singleton_items_info, const std::vector<std::string> &search_paths)
	...
	std::string texture_vert_code = util::read_whole_file(
		static_cast<GameSingletonsInfo*>(singleton_items_info)->data_dir
	...
	auto plaintexture_vert = std::make_unique<shader::Shader>(
		...
		std::initializer_list<const char *>{
			shader_header_code,
			texture_vert_code.c_str()
		}
	);

	std::string texture_frag_code = util::read_whole_file(
		static_cast<GameSingletonsInfo*>(singleton_items_info)->data_dir
		+ "/shaders/maptexture.frag.glsl"
	);
	auto plaintexture_frag = std::make_unique<shader::Shader>(
		GL_FRAGMENT_SHADER,
		std::initializer_list<const char *>{
			shader_header_code,
			texture_frag_code.c_str()
		}
	);
