...
camgame_delta phys3_delta::to_camgame(const CoordManager &mgr) const {
	...
	//  (x) = (+1 +1 +0) (se)
	//  (y) = (+1 -1 +1) (up)
	phys_t x = this->ne * (+1) + this->se * (+1) + this->up * (+0);
	phys_t y = this->ne * (-1) + this->se * (+1) + this->up * (+1);

	// add scaling (w/2 for x, h/2 for y)
	return camgame_delta{
		static_cast<camgame_delta::elem_t>((x * mgr.tile_halfsize.x).to_int()),
		static_cast<camgame_delta::elem_t>((y * mgr.tile_halfsize.y).to_int())
	};
}
...
window phys3::to_window(const CoordManager &mgr) const {
	return this->to_camgame(mgr).to_window(mgr);
}
