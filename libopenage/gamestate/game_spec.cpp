...
bool GameSpec::initialize()
{
	...
	std::vector<gamedata::string_resource> string_resources =
			util::read_csv_file<gamedata::string_resource>(
	...
	} catch (std::exception &exc)
	{
		...
		throw Error
		{ ERR << "gamedata could not be loaded: "
				<< util::demangle(typeid(exc).name()) << ": " << exc.what() };
	}

	log::log(
			MSG(info).fmt("Loading time  [data]: %5.3f s",
...
Texture* GameSpec::get_texture(index_t graphic_id) const
{
	...
	std::string tex_fname = util::sformat("converted/graphics/%d.slp.png",
			slp_id);

	return this->get_texture(tex_fname, true);
}

Texture* GameSpec::get_texture(const std::string &file_name,
		bool use_metafile) const
{
...
std::vector<const gamedata::unit_command*> GameSpec::get_command_data(
		index_t unit_id) const
{
...
void GameSpec::on_gamedata_loaded(const gamedata::empiresdat &gamedata)
{
	...
	{
		this->unit_textures.insert(
		{ g.first, std::make_shared<UnitTexture>(*this, g.second) });
	}
	...
	for (const gamedata::sound &sound : gamedata.sounds.data)
	{
		...
		for (const gamedata::sound_item &item : sound.sound_items.data)
		{
			...
			std::string snd_filename = util::sformat("%d.opus",
					item.resource_id);
			...
			{ audio::category_t::GAME, item.resource_id, snd_path,
					audio::format_t::OPUS, audio::loader_policy_t::DYNAMIC };
	...
	audio::AudioManager &am =
			this->assetmanager->get_engine()->get_audio_manager();
...
void GameSpec::load_building(const gamedata::building_unit &building,
		unit_meta_list &list) const
{
	...
	{
		auto meta_type =
				std::make_shared<UnitTypeMeta>("Building", building.id0,
						[this, &building](
								const Player &owner)
								{
...
void GameSpec::load_living(const gamedata::living_unit &unit,
		unit_meta_list &list) const
{
	...
	{
		auto meta_type =
				std::make_shared<UnitTypeMeta>("Living", unit.id0,
						[this, &unit](
								const Player &owner)
								{
...
void GameSpec::load_object(const gamedata::unit_object &object,
		unit_meta_list &list) const
{
	...
	{
		auto meta_type =
				std::make_shared<UnitTypeMeta>("Object", object.id0,
						[this, &object](
								const Player &owner)
								{
...
void GameSpec::load_missile(const gamedata::missile_unit &proj,
		unit_meta_list &list) const
{
	...
	{
		auto meta_type =
				std::make_shared<UnitTypeMeta>("Projectile", proj.id0,
						[this, &proj](
								const Player &owner)
								{
...
void GameSpec::load_terrain(const gamedata::empiresdat &gamedata)
{
	...
	std::vector<gamedata::blending_mode> blending_meta = util::read_csv_file<
			gamedata::blending_mode>(convert_dir["blending_modes.docx"]);
	...
	log::log(
			MSG(dbg) << "Terrain prefs: " << "tiletypes="
					<< terrain_data.terrain_id_count << ", "
							"blendmodes=" << terrain_data.blendmode_count);

	// create tile textures (snow, ice, grass, whatever)
	for (size_t terrain_id = 0; terrain_id < terrain_data.terrain_id_count;
			terrain_id++)
	{
		...
		auto new_texture = this->assetmanager->get_texture(terraintex_filename,
				true);
	...
	for (size_t i = 0; i < terrain_data.blendmode_count; i++)
	{
		...
		std::string mask_filename = util::sformat(
				"converted/blendomatic/mode%02d.png", line->blend_mode);
		terrain_data.blending_masks[i] = this->assetmanager->get_texture(
				mask_filename);
	}
}
...
void Sound::play() const
{
	...
	try
	{
		...
		audio::AudioManager &am =
				this->game_spec->get_asset_manager()->get_engine()->get_audio_manager();
...
GameSpecHandle::GameSpecHandle(qtsdl::GuiItemLink *gui_link) :
		...
		{ }, asset_manager
		{ }, gui_signals
		{ std::make_shared<GameSpecSignals>() }, gui_link
		{ gui_link }
{
}
...
void GameSpecHandle::announce_spec()
{
	...
	emit
	this->gui_signals->game_spec_loaded(this->spec);
}
...
void GameSpecHandle::start_load_job()
{
	...
	auto spec_and_job = std::make_tuple(this->spec, this->gui_signals,
			job::Job<bool>
			{ });
	auto spec_and_job_ptr = std::make_shared<decltype(spec_and_job)>(
			spec_and_job);
	...
	auto load_finished = [gui_signals_ptr = this->gui_signals.get()](
			job::result_function_t<bool> result)
			{
				bool load_ok;
				try
				{
	...
	job::JobManager *job_mgr =
			this->asset_manager->get_engine()->get_job_manager();
