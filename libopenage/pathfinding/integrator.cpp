...
std::shared_ptr<IntegrationField> Integrator::integrate(const std::shared_ptr<CostField> &cost_field,
	...
	if (with_los) {
		wavefront_blocked = integration_field->integrate_los(cost_field, other_sector_id, portal, target);
	}
...
Integrator::build_return_t Integrator::build(const std::shared_ptr<CostField> &cost_field,
	...
	auto integration_field = this->integrate(cost_field, other_sector_id, portal, target, with_los);
