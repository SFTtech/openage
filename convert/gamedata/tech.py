...
class TechData:
    def read(self, raw, offset):
        ...
        self.tech_count = pc[0]

        self.tech = list()
        for i in range(self.tech_count):
            t = Tech()
...
class Tech:
    def read(self, raw, offset):
        ...
        self.name         = zstr(pc[0])
        self.effect_count = pc[1]

        self.effect = list()
        for i in range(self.effect_count):
            t = Effect()
            offset = t.read(raw, offset)
            self.effect.append(t)
...
class TechtreeData:
    def read(self, raw, offset):
        ...
        self.age_entry_count           = header[0]
        self.building_connection_count = header[1]
        self.unit_connection_count     = header[2]
        self.research_connection_count = header[3]

        self.age_tech_tree = list()
        for i in range(self.age_entry_count):
            dbg("=== age_tech_tree %d" % i, 2)
            t = AgeTechTree()
            offset = t.read(raw, offset)
            self.age_tech_tree.append(t)

        tmp_struct = Struct(endianness + "i")
        offset += tmp_struct.size

        self.building_connection = list()
        for i in range(self.building_connection_count):
            dbg("=== building_connection %d" % i, 2)
            t = BuildingConnection()
            offset = t.read(raw, offset)
            self.building_connection.append(t)

        self.unit_connection = list()
        for i in range(self.unit_connection_count):
            dbg("=== unit_connection %d" % i, 2)
            t = UnitConnection()
            offset = t.read(raw, offset)
            self.unit_connection.append(t)

        self.research_connection = list()
        for i in range(self.research_connection_count):
            dbg("=== research_connection %d" % i, 2)
            t = ResearchConnection()
            offset = t.read(raw, offset)
            self.research_connection.append(t)
...
class AgeTechTree:
    def read(self, raw, offset):
        ...
        self.uid            = pc[1]
        #self. = pc[2]
        self.building_count = pc[3]

        #int32_t building[building_count];
        age_tech_tree_struct = Struct(endianness + "%di" % self.building_count)
        pc = age_tech_tree_struct.unpack_from(raw, offset)
        ...
        age_tech_tree_struct = Struct(endianness + "%di" % self.unit_count)
        pc = age_tech_tree_struct.unpack_from(raw, offset)
        ...
        offset += age_tech_tree_struct.size
        self.research_count = pc[0]

        #int32_t research[research_count];
        age_tech_tree_struct = Struct(endianness + "%di" % self.research_count)
        pc = age_tech_tree_struct.unpack_from(raw, offset)
...
class BuildingConnection:
    def read(self, raw, offset):
        ...
        self.uid            = pc[0]
        #self. = pc[1]
        self.building_count = pc[2]

        #int32_t building[building_count];
        building_connection_struct = Struct(endianness + "%di" % self.building_count)
        pc = building_connection_struct.unpack_from(raw, offset)
        ...
        #int32_t unit[unit_count];
        building_connection_struct = Struct(endianness + "%di" % self.unit_count)
        pc = building_connection_struct.unpack_from(raw, offset)
        ...
        #int32_t research[research_count];
        building_connection_struct = Struct(endianness + "%di" % self.research_count)
        pc = building_connection_struct.unpack_from(raw, offset)
        ...
        self.age               = pc[0]
        self.unit_or_research0 = pc[1]
        self.unit_or_research1 = pc[2]
        #self. = pc[3:(3+8)]
        self.mode0             = pc[11]
        self.mode1             = pc[12]
        #self. = pc[13:(13+7)]
        #self. = pc[20:(20+11)]
        self.connection        = pc[31]
        self.enabling_research = pc[32]
...
class UnitConnection:
    def read(self, raw, offset):
        ...
        self.uid                  = pc[0]
        #self. = pc[1]
        self.upper_building      = pc[2]
        self.required_researches = pc[3]
        self.age                 = pc[4]
        self.unit_or_research0   = pc[5]
        self.unit_or_research1   = pc[6]
        #self. = pc[7:(7+8)]
        self.mode0               = pc[15]
        self.mode1               = pc[16]
        #self. = pc[17:(17+7)]
        self.vertical_line       = pc[24]
        self.unit_count          = pc[25]

        #int32_t unit[unit_count];
        unit_connection_struct = Struct(endianness + "%di" % self.unit_count)
        pc = unit_connection_struct.unpack_from(raw, offset)
        ...
        self.location_in_age   = pc[0]
        self.required_research = pc[1]
        self.line_mode         = pc[2]
        self.enabling_research = pc[3]
...
class ResearchConnection:
    def read(self, raw, offset):
        ...
        #int32_t building[building_count];
        research_connection_struct = Struct(endianness + "%di" % self.building_count)
        pc = research_connection_struct.unpack_from(raw, offset)
        ...
        #int32_t unit[unit_count];
        research_connection_struct = Struct(endianness + "%di" % self.unit_count)
        pc = research_connection_struct.unpack_from(raw, offset)
        ...
        #int32_t research[research_count];
        research_connection_struct = Struct(endianness + "%di" % self.research_count)
        pc = research_connection_struct.unpack_from(raw, offset)
        ...
        self.required_research = pc[0]
        self.age = pc[1]
        self.upper_research = pc[2]
        #self. = pc[3:(3+9)]
        self.line_mode = pc[12]
        #self. = pc[13:(13+8)]
        self.vertical_line = pc[21]
        self.location_in_age = pc[22]
        #self. = pc[23]
