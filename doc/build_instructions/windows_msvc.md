...
## Running openage (in devmode)
 While this is straightforward on other platforms, there is still stuff to do to run openage on Windows:
  ...
  - Now, to run the openage:
    - Open a CMD window in `<openage directory>` and run `movepyd.cmd <your python site-packages directory>` (Usually something like `C:\Users\YourUsernameHere\AppData\Local\Programs\Python\<relevant python installation>\Lib\site-packages`)
