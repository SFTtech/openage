...
- Add `condition : set(Clause)` member to `RefundOnCondition(Modifier)`
...
## Changed
### Ability module
- Change type of `empty_threshold` member in `Storage` from `AttributeAmount` to `set(Clause)` and rename it to `empty_condition`
...
- Add `progress: int` member from `Progress(Entity)`; progress is now defined by bounded interval with members `left_boundary : float`, `right_boundary : float`
